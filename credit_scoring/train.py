...
def save_model(model: CreditRNNModel, type: str, metrics: Dict = None) -> None:
...
def train_model(
    ...
    msg = f"Starting training of the model on " f"{num_epochs} epochs.\n"
    print(msg)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if roc_auc > max_roc:
            max_roc = roc_auc
            best_checkpoint = {"epoch": epoch, "roc_auc": roc_auc, "loss": epoch_loss}
            save_model(model, "best")
        history = {"losses": loss_list, "roc-auc": roc_list}
        msg = (
            f"Trained epoch {epoch} with loss "
...
def evaluate(model: CreditRNNModel, criterion: Callable, dataloader: DataLoader) -> float:
    model.eval()
    roc_auc = []
    with torch.no_grad():
...
def train_pipeline(
    ...
    # Preprocessing
    paths_dict = PathsDict().make(data_root, train_folder, train_target, test_folder)
    ...
    tr_features, val_features, tr_target, val_target = train_test_split(
        list(all_features.values()), tr_target["flag"].values, test_size=VAL_SIZE
    )
    ...
    model = CreditRNNModel(
        seq_len=n_features, aggregation_type=agg_type, rnn_type=rnn_type
    )
